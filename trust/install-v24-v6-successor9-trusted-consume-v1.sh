...
test "$(jq -r '.schema_version' "$MANIFEST")" = "5"
test "$(jq -r '.file_hash_mode' "$MANIFEST")" = "git_blob_sha1"
test "$(jq -r '.manifest_id' "$MANIFEST")" = "V24-V6-SUCCESSOR9-TRUSTED-CONSUME-BOOTSTRAP-V1"
test "$(jq -r '.service_id' "$MANIFEST")" = "V24-V6-TRUSTED-AUTHORITY-SERVICE"
test "$(jq -r '.service_version' "$MANIFEST")" = "3"
...
test "$(jq -r '.trusted_consume_protocol' "$MANIFEST")" = "V24-V6-S9-CONSUME/1"
test "$(jq -r '.consumption_mode' "$MANIFEST")" = "ROOT_PEER_SEMANTIC_ONE_SHOT"
...
STAGE="$(mktemp -d /var/tmp/v24-v6-s9-bootstrap.XXXXXX)"
...
env -i PATH=/usr/bin:/bin LC_ALL=C.UTF-8 \
  bash "$STAGE/build_v24_v6_external_authority_gate.sh" >/var/tmp/v24-s9-gate-build.log
env -i PATH=/usr/bin:/bin LC_ALL=C.UTF-8 \
  bash "$STAGE/build_v24_v6_trusted_authority_service.sh" >/var/tmp/v24-s9-service-build.log
...
install -o root -g root -m 0444 "$MANIFEST" \
  "$TRUSTED_ROOT/bootstrap/v24-v6-successor9-trusted-consume-bootstrap-v1.json"
...
chmod 0444 "$TRUSTED_ROOT/bootstrap/v24-v6-successor9-trusted-consume-bootstrap-v1.json"
...
test "$(git hash-object "$TRUSTED_ROOT/bootstrap/v24-v6-successor9-trusted-consume-bootstrap-v1.json")" = "$EXPECTED_MANIFEST_BLOB_SHA1"
