...
EXPECTED_MANIFEST_BLOB_SHA1="bf42c039956ccfdabcd127672482c0f30f698010"
...
test "$(jq -r '.manifest_id' "$MANIFEST")" = "V24-V6-SUCCESSOR8-AUTHORITY-RECORD-BOOTSTRAP-V1"
test "$(jq -r '.service_id' "$MANIFEST")" = "V24-V6-TRUSTED-AUTHORITY-SERVICE"
test "$(jq -r '.service_version' "$MANIFEST")" = "$EXPECTED_SERVICE_VERSION"
...
test "$(jq -r '.predecessor_review_commit' "$MANIFEST")" = "3cb3a9efb82f58bca9fb7734b285e2e2d8a2bde1"
test "$(jq -r '.preserved_red_run_id' "$MANIFEST")" = "35342416032"
...
env -i PATH=/usr/bin:/bin LC_ALL=C.UTF-8 \
  bash "$STAGE/build_v24_v6_external_authority_gate.sh" >/var/tmp/v24-s8-gate-build.log
env -i PATH=/usr/bin:/bin LC_ALL=C.UTF-8 \
  bash "$STAGE/build_v24_v6_trusted_authority_service.sh" >/var/tmp/v24-s8-service-build.log
...
install -o root -g root -m 0444 "$MANIFEST" \
  "$TRUSTED_ROOT/bootstrap/v24-v6-successor8-authority-record-bootstrap-v1.json"
...
chmod 0444 "$TRUSTED_ROOT/bootstrap/v24-v6-successor8-authority-record-bootstrap-v1.json"
...
test "$(git hash-object "$TRUSTED_ROOT/bootstrap/v24-v6-successor8-authority-record-bootstrap-v1.json")" = "$EXPECTED_MANIFEST_BLOB_SHA1"

gate_identity="$("$TRUSTED_ROOT/.gate-build/v24_v6_external_authority_gate" --identity)"
service_identity="$("$TRUSTED_ROOT/v24_v6_trusted_authority_service" --identity)"
test "$(printf '%s' "$gate_identity" | jq -r '.gate_version')" = "1"
test "$(printf '%s' "$service_identity" | jq -r '.service_version')" = "$EXPECTED_SERVICE_VERSION"
printf '%s\n' "$gate_identity"
printf '%s\n' "$service_identity"
