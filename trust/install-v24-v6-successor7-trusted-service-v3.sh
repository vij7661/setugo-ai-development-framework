...
EXPECTED_MANIFEST_BLOB_SHA1="ccfa227d511934b8c5068ee395604f06d7097aed"
...
test "$(jq -r '.schema_version' "$MANIFEST")" = "3"
test "$(jq -r '.file_hash_mode' "$MANIFEST")" = "git_blob_sha1"
test "$(jq -r '.manifest_id' "$MANIFEST")" = "V24-V6-SUCCESSOR7-TRUSTED-SERVICE-BOOTSTRAP-V3"
...
env -i PATH=/usr/bin:/bin LC_ALL=C.UTF-8   bash "$STAGE/build_v24_v6_external_authority_gate.sh" >/var/tmp/v24-s7-gate-build.log
env -i PATH=/usr/bin:/bin LC_ALL=C.UTF-8   bash "$STAGE/build_v24_v6_trusted_authority_service.sh" >/var/tmp/v24-s7-service-build.log
...
install -o root -g root -m 0444 "$MANIFEST"   "$TRUSTED_ROOT/bootstrap/v24-v6-successor7-trusted-service-bootstrap-v3.json"
...
chmod 0444 "$TRUSTED_ROOT/bootstrap/v24-v6-successor7-trusted-service-bootstrap-v3.json"
...
test "$(git hash-object "$TRUSTED_ROOT/bootstrap/v24-v6-successor7-trusted-service-bootstrap-v3.json")" = "$EXPECTED_MANIFEST_BLOB_SHA1"
