...
class ECCV10FilesystemPrimitiveIntegrity(unittest.TestCase):
    ...
    def test_path_read_text_substitution_plus_dependency_tamper_cannot_mint(self):
        ...
        forged_text = json.dumps(forged_manifest)
    ...
    def test_path_read_bytes_alias_substitution_does_not_control_legitimate_authority(self):
        with patch.object(boundary.Path, "read_bytes", return_value=b"forged-file-bytes"):
            result = self.genuine_result()
        self.assertEqual(result["status"], "VERIFIED")
        self.assertTrue(boundary.candidate_result_eligible(result))

    def test_path_resolve_alias_substitution_does_not_control_legitimate_authority(self):
        with patch.object(boundary.Path, "resolve", side_effect=OSError("forged resolve")):
            result = self.genuine_result()
        self.assertEqual(result["status"], "VERIFIED")
        self.assertTrue(boundary.candidate_result_eligible(result))

    def test_path_is_file_alias_substitution_does_not_control_legitimate_authority(self):
        with patch.object(boundary.Path, "is_file", return_value=False):
            result = self.genuine_result()
        self.assertEqual(result["status"], "VERIFIED")
        self.assertTrue(boundary.candidate_result_eligible(result))

    def test_path_is_symlink_alias_substitution_does_not_control_legitimate_authority(self):
