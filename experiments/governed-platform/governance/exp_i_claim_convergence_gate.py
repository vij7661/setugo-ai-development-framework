...
def _invalid(reason: str) -> GateDecision:
    return GateDecision(
...
def evaluate_review_gate(
    ...
    if len(reviews) != REQUIRED_REVIEWER_COUNT:
        return _invalid(f"exactly {REQUIRED_REVIEWER_COUNT} review claims required")
    ...
    if len(reviewer_ids) != REQUIRED_REVIEWER_COUNT:
        return _invalid("exactly three distinct reviewer_id values required")

    case_ids = {review.case_id for review in reviews}
    if len(case_ids) != 1:
        return _invalid("review case_id mismatch")
    ...
    return GateDecision(
        ...
        canonical_primary_failure_class=canonical_primary,
        canonical_artifact_scope=canonical_scope,
        reasons=("exact three-reviewer consensus plus matching platform-owned verification",),
    )
