...
def authorize_policy_bound_execution(authority: dict, current: dict, request: dict) -> dict:
    ...
    if authority["artifact_hash"] != current.get("artifact_hash"):
        return {"authorized": False, "reason": "current artifact changed after authorization"}
    if authority["artifact_hash"] != request.get("artifact_hash"):
        return {"authorized": False, "reason": "request artifact changed after authorization"}
    if request.get("artifact_hash") != current.get("artifact_hash"):
        return {"authorized": False, "reason": "request bound to stale artifact"}
    ...
    return {"authorized": True, "reason": "authority, current policy/artifact, request and capability remain exactly bound"}
