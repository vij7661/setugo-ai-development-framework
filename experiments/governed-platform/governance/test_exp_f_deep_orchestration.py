...
def event(event_id="evt-a", **overrides):
    value = {
        ...
        "conclusion": "success",
        "evidence_refs": ["judge:canonical", "regression:canonical"],
...
class ExpFDeepOrchestrationTests(unittest.TestCase):
    def test_sequentialized_race_loser_cannot_complete_again(self):
        first = process_event(state(), event("evt-a"))
        self.assertEqual("COMPLETE", first["decision"])
        second = process_event(first["state"], event("evt-b", expected_state_version=5))
        self.assertEqual("IGNORE", second["decision"])
        self.assertIn("state-version race", second["reason"])

    def test_cross_project_evidence_binding_cannot_be_laundered(self):
        bad = state()
        bad["evidence_bindings"]["judge:canonical"]["project_id"] = "other-project"
        result = process_event(bad, event(requested_transition="CONTINUING"))
        self.assertEqual("BLOCK", result["decision"])
        self.assertIn("provenance", result["reason"])

    def test_cross_task_evidence_binding_cannot_be_laundered(self):
        bad = state()
        bad["evidence_bindings"]["judge:canonical"]["task_id"] = "other-task"
        result = process_event(bad, event(requested_transition="CONTINUING"))
        self.assertEqual("BLOCK", result["decision"])
    ...
    def test_alias_evidence_identity_cannot_substitute_for_canonical_refs(self):
        result = process_event(
            state(),
            event(evidence_refs=["judge:canonical-copy", "regression:canonical"]),
        )
        self.assertEqual("BLOCK", result["decision"])
    ...
    def test_asserted_compatibility_cannot_bypass_invariant_extraction(self):
        ...
        candidate = {
            "asserted_compatible": True,
            "preserved_invariants": ["authority_external"],
        }
        result = authorize_model_routing(contract, candidate)
        self.assertFalse(result["authorized"])
        self.assertEqual(["evidence_before_promotion"], result["gate"]["missing_invariants"])

    def test_routing_fails_closed_when_contract_omits_invariants(self):
        with self.assertRaises(ValueError):
            authorize_model_routing({}, {"preserved_invariants": []})
