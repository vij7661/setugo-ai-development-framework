"""EXP-O Pilot 11 process harness/client for ownership-fenced semantic permits."""
...
class FencedSemanticLoopbackClient(SemanticLoopbackClient):
...
class FencedSemanticGatewayProcessHarness:
    def __init__(
        self,
        *,
        ...
        self.lease_db_path = Path(lease_db_path)
        ...
        self.lease_key = bytes(lease_key)
    ...
    def script_path(self) -> Path:
        return Path(__file__).with_name("mcp_semantic_fenced_gateway_process_exp_o.py")

    def start(self, *, timeout_s: float = 5.0) -> dict[str, Any]:
        if self.process is not None and self.process.poll() is None:
            raise RuntimeError("fenced semantic gateway process already running")
        ...
        env["EXP_O_SEMANTIC_LEASE_KEY_HEX"] = self.lease_key.hex()
        env["EXP_O_INNER_PERMIT_KEY_HEX"] = self.inner_permit_key.hex()
        args = [
            sys.executable,
            str(self.script_path),
            "--effects-db", str(self.effects_db_path),
            "--registry-db", str(self.registry_db_path),
            "--lease-db", str(self.lease_db_path),
            "--ready-file", str(self.ready_path),
            "--clock-file", str(self.clock_path),
        ]
        ...
        while time.monotonic() < deadline:
            if self.process.poll() is not None:
                ...
                self.process = None
                raise RuntimeError(f"fenced semantic gateway exited early with {code}")
            if self.ready_path.exists():
                self.info = json.loads(self.ready_path.read_text(encoding="utf-8"))
                return dict(self.info)
            time.sleep(0.02)
        self.stop()
        raise TimeoutError("fenced semantic gateway did not become ready")

    @property
    def base_url(self) -> str:
        if not self.info:
            raise RuntimeError("fenced semantic gateway not started")
        return f"http://127.0.0.1:{int(self.info['port'])}"

    def client(self, *, timeout_s: float = 2.0) -> FencedSemanticLoopbackClient:
        return FencedSemanticLoopbackClient(self.base_url, timeout_s=timeout_s)
    ...
    def __enter__(self) -> "FencedSemanticGatewayProcessHarness":
        self.start()
        return self
