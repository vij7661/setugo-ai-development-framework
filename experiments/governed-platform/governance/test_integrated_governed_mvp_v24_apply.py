...
class V24GatewayIntegrationTests(unittest.TestCase):
 def setUp(self):
  ...
  self.route={"provider":"groq","model":"model-a","sku":"sku-a","deployment_path":"api-a","qualification_ref":"q-1","qualification_epoch":7};self.registry={**self.route,"qualification_expires_epoch":100,"eligible":True,"revoked":False}
  self.cap={"capability_id":"cap-1","project_id":"p-1","task_id":"t-1","subject_id":"worker-1","issued_epoch":3,"expires_at":"2030-01-01T00:00:00Z","allowed_actions":["WRITE"],"artifact_classes":["CODE"],"revoked":False};self.req={"capability_id":"cap-1","project_id":"p-1","task_id":"t-1","subject_id":"worker-1","issued_epoch":3,"action":"WRITE","artifact_classes":["CODE"]}
 ...
 def gateway(self,provider=None):return V24ExecutionGateway(self.db,self.worker,current_context_provider=provider or (lambda c:deepcopy(dict(c))))
 def upstream(self):
 ...
 def kwargs(self,key="idem-1"):return {"upstream":self.upstream(),"current_registry_entry":deepcopy(self.registry),"current_capability":deepcopy(self.cap),"execution_request":deepcopy(self.req),"current_artifact_binding":deepcopy(self.art),"idempotency_key":key,"now_epoch":11,"now_iso":"2026-09-07T00:01:00Z"}
 def test_v24_positive_executes_once_and_returns_application_record(self):
  g=self.gateway();r=g.execute(v24_apply_context=self.ctx(),**self.kwargs());self.assertEqual(r["state"],"EXECUTED");self.assertEqual(self.calls,1);self.assertEqual(r["v24_guard"]["state"],"V24_APPLY_READY");self.assertEqual(r["authority_application_record"]["decision_digest"],"D"*64)
 def test_unqualified_v24_prerequisite_blocks_before_gateway_effect(self):
  g=self.gateway();c=self.ctx();c["normative_catalog_state"]="PENDING";r=g.execute(v24_apply_context=c,**self.kwargs());self.assertEqual(r["state"],"V24_APPLY_BLOCKED");self.assertEqual(self.calls,0);self.assertEqual(g.effect_count(),0)
 def test_stale_ledger_blocks(self):
  g=self.gateway();c=self.ctx();c["current_admission_ledger_digest"]="OLD";r=g.execute(v24_apply_context=c,**self.kwargs());self.assertEqual(r["state"],"V24_APPLY_BLOCKED");self.assertEqual(self.calls,0)
 def test_unadmitted_writer_blocks(self):
  g=self.gateway();c=self.ctx();c["admission_perimeter_record"]["admitted_writer_ids"]=[];r=g.execute(v24_apply_context=c,**self.kwargs());self.assertEqual(r["state"],"V24_APPLY_BLOCKED");self.assertEqual(self.calls,0)
 def test_fresh_provider_change_blocks_at_final_boundary(self):
  calls={"n":0}
  def provider(c):
   calls["n"]+=1;out=deepcopy(dict(c))
   if calls["n"]>=2:out["current_completeness_ledger_digest"]="DRIFT"
   return out
  g=self.gateway(provider);r=g.execute(v24_apply_context=self.ctx(),**self.kwargs());self.assertEqual(r["state"],"V24_APPLY_BLOCKED");self.assertEqual(self.calls,0);self.assertEqual(g.effect_count(),0)
 def test_replay_preserves_exact_application_record_without_duplicate_effect(self):
  g=self.gateway();a=g.execute(v24_apply_context=self.ctx(),**self.kwargs());b=g.execute(v24_apply_context=self.ctx(),**self.kwargs());self.assertEqual(b["state"],"REPLAYED");self.assertEqual(a["authority_application_record"],b["authority_application_record"]);self.assertEqual(self.calls,1);self.assertEqual(g.effect_count(),1)
