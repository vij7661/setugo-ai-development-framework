"""OpenAI-compatible adapter for the external-authority experiment path.

This module is isolated from the legacy adapter so EXP-N Pilot 8 can finish on its
pre-registered prompt and runner code unchanged.
"""
...
from review_contract_external_authority import EXTERNAL_AUTHORITY_REVIEW_OUTPUT_SCHEMA_INSTRUCTION

RETRYABLE_HTTP = frozenset({429, 500, 502, 503, 504})


@dataclass(frozen=True)
class ExternalAuthorityRemoteProviderConfig:
    provider_id: str
    base_url: str
    model: str
    api_key_env: str
...
class OpenAICompatibleExternalAuthorityAdapter(MechanismAdapter):
    def __init__(self, config: ExternalAuthorityRemoteProviderConfig) -> None:
        ...
        for attempt in range(1, self._config.max_attempts + 1):
            request = Request(
                ...
                headers={
                    ...
                    "User-Agent": "setugo-governed-platform-external-authority/1.0",
                },
                method="POST",
            )
            try:
            ...
            if finish_reason != "stop":
                raise RuntimeError(
                    f"provider completion is incomplete or nonterminal: finish_reason={finish_reason!r}"
                )
