# V24 I11 Systemic Remediation Design V6 — Clean Review Packet

**PLAN_BODY_SHA256:** `286217399947144630c29e991c3529dfee64d382be08a6ccd3f87e9fc5173ad5`  
...
# V24 I11 Systemic Remediation Design V6 — Clean Independent Review Surface
...
Review only this V6 packet. Do not import prior findings, adjudications, chats, model memory, or implementation proposals.
...
### 6.3 Acyclicity and universal allowed-root closure

`CompletenessDerivationGraphVerifier` is an ABGOU qualified mechanism and must prove all of the following over the exact dependency graph:
- no self-edge;
- no directed cycle;
- no strongly connected component of size > 1;
- every terminal node (out-degree zero under the dependency relation) is an allowed root from Section 6.2;
- every root/source-surface reference appearing anywhere in the graph is an allowed root from Section 6.2;
- **every** direct and transitive dependency path from every omission-sensitive subject terminates **only** in allowed roots;
- no node depends directly or transitively on any non-allowed root/source surface;
- no expected-universe derivation depends directly or transitively on the subject it is qualifying.

Formally, for every subject node `s`, if `ReachableTerminals(s)` is the set of terminal dependency nodes reachable from `s`, qualification requires:

`ReachableTerminals(s) != ∅ AND ReachableTerminals(s) ⊆ AllowedRoots`

and every external/source reference used by the derivation must be represented as one of those allowed-root nodes. A path that reaches one allowed root but also reaches any disallowed terminal/source is invalid.
...
GenesisTrustedABGOUScopeRecord {
  ...
  root_kernel_digest
  trusted_objects[]  # ordered canonical records: {object_id, content_digest}
...
Residual root trust is granted only by exact paired membership in `trusted_objects[]`. The verifier must find one canonical entry whose `object_id` equals the subject object ID **and whose `content_digest` in that same entry** equals the subject content/implementation digest. Independent membership in separate ID/digest sets is forbidden.

`trusted_object_pair_set_digest` is the canonical digest over the ordered set of exact `{object_id, content_digest}` pairs and is bound into `scope_digest`. Cross-matching an ID from one trusted pair with a digest from another pair is exactly `GENESIS_TRUST_SCOPE_MISMATCH_REJECTED`.
...
`ApplicablePredicateUniverse` is explicitly an omission-sensitive ABGOU universe. Before it can drive coverage, it requires a `RegistryCompletenessQualificationRecord` whose independently derived expected-member set is the exact predicate-applicability obligation set produced from the complete qualified endpoint-predicate universe, qualified applicability rules/compiler, exact decision-context class, and admitted subsystem overrides. Exact set equality is required; an omitted applicable predicate blocks qualification before endpoint projection.

Evaluator contracts, condition schemas, evidence classes, coverage records, and coverage verifier are also ABGOU members. One evaluation per applicable predicate is required; FALSE requires governed negative evidence; TRUE requires typed condition(s); producer-chosen N/A is forbidden.
...
Atomic modes are stored in an omission-sensitive `AtomicBindingModeRegistry` and are ABGOU governed objects. The registry must itself have a current `RegistryCompletenessQualificationRecord` proving exact set equality against an independently derived atomic-binding-mode obligation set from the active condition/evaluation binding contracts and admitted transaction/snapshot mechanisms. A mode omitted from either side makes the registry non-qualified.
...
When any unresolved case later resolves, its new result record must bind the exact qualification/evidence digests that made execution/adjudication newly admissible. Historical unresolved records remain preserved. A later resolution **never retroactively converts the original unresolved record or original qualification into PASS**. PASS can count only in a new qualification/adjudication that re-establishes the exact candidate commit/tree, environment, case contract, and then-current qualification/evidence basis.

## 17. Review convergence / severity

A `Critical` design finding must state:
1. exact authority-bearing object/transition;
2. which inherited V6 contracts apply;
...
Only after exact V6 clean approval may implementation branch from frozen I10 lineage.
...
## 18A. Mandatory V6 adversarial review checks

The reviewer must explicitly test these invariants:
- a derivation with one allowed root **and one disallowed terminal/source** must be rejected;
- every reachable terminal from every completeness subject must be in `AllowedRoots`;
- genesis trust must fail if object ID and digest come from different trusted pairs;
- omission of one applicable predicate from `ApplicablePredicateUniverse` must fail completeness qualification;
- omission of one required atomic binding mode must fail `AtomicBindingModeRegistry` completeness;
- later resolution of an unresolved case must not alter the historical qualification/PASS count.
