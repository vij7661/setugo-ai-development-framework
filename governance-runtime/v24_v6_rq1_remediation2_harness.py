...
CASES = {
 ...
 "RQ-05":("Install/start/consume rejected","path-substitution"),
 "RQ-06":("Bootstrap/service rejected","candidate-environment"),
 "RQ-07":("External pin verification rejects it","source-manifest-substitution"),
 "RQ-11":("Exactly one success; all others replay/unavailable","concurrent-consumers"),
 "RQ-12":("Rejected","replay-after-restart"),
 "RQ-13":("No consume or authoritative result","crash-before-validation"),
 "RQ-14":("Recoverable without duplicate success","crash-after-validation-before-rename"),
 "RQ-15":("One consumed state; never a second success","crash-after-rename"),
 "RQ-16":("Fail closed without fabricated success","fault-injection"),
 "RQ-17":("Configuration rejected","cross-filesystem"),
 "RQ-18":("Startup or qualification gate fails","binary-manifest-mismatch"),
 "RQ-19":("Qualification invalidated","dependency-drift"),
 "RQ-20":("Qualification invalidated","unit-environment-drift"),
 "RQ-21":("Startup or qualification gate fails","ownership-mode-drift"),
 "RQ-26":("DENY","stale-decision-substitution"),
 "RQ-27":("DENY","cross-control-composition"),
 "RQ-29":("No bypass; bounded recovery","resource-exhaustion"),
 "RQ-30":("Rejected or qualification invalidated","unpinned-upgrade-rollback"),
 "RQ-31":("No authority from absent evidence","logging-telemetry-failure"),
 "RQ-32":("Consistent state; replay still rejected","reboot-cold-start"),
}
...
def exact_boundary_target(cid, target_name, trace):
...
def case_07(out):
    ...
    verify=run(["/opt/v24-v6-trusted-runtime/.gate-build/v24_v6_external_authority_gate","resolve-governed","/tmp/rq07-context-missing","/tmp/rq07-boundary-missing","0"*64,"candidate-id","0"*64,"V24-V6-EXTERNAL-AUTHORITY-GATE","1","enforce"],user="root",timeout=15) if install.returncode==0 else install
...
def _crash_case(cid,out,boundary):
    ...
    post_replay=observe(out/f"{cid}.post-replay.observer.json")
    post_retry_life=target_lifecycle(post_retry,target_name)
    post_replay_life=target_lifecycle(post_replay,target_name)
    prepared_records_delta=set_delta(baseline,prepared,"records")
    prepared_consumed_delta=set_delta(baseline,prepared,"consumed")
    recovery_records_delta=set_delta(prepared,recovery,"records")
    recovery_consumed_delta=set_delta(prepared,recovery,"consumed")
    retry_records_delta=set_delta(recovery,post_retry,"records")
