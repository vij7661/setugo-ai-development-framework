...
The committed manifest binds R1-R8 by exact Git blob identity. This verifier
also recomputes raw SHA-256 from the same bytes for the verification report.
It is construction-only and cannot open scientific execution or grant authority.
"""
...
def validate_integrated_successor_manifest(*, repo_root: str | Path, manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for key in ("candidate_family_id", "approved_design_packet_sha256", "approved_design_body_sha256", "frozen_i10_commit", "frozen_i10_tree", "implementation_lineage_root_commit"):
    ...
    if not isinstance(rows, list):
        rows = []; p.append("INTEGRATED_SUCCESSOR_WORKSTREAMS_REQUIRED")
    by_id: dict[str, Mapping[str, Any]] = {}
    for i, row in enumerate(rows):
        if not isinstance(row, Mapping):
            p.append(f"INTEGRATED_SUCCESSOR_WORKSTREAM_MALFORMED:{i}"); continue
        wid = row.get("workstream_id")
        if wid not in EXPECTED_WORKSTREAMS:
            p.append(f"INTEGRATED_SUCCESSOR_WORKSTREAM_ID_INVALID:{wid}"); continue
        if wid in by_id:
            p.append(f"INTEGRATED_SUCCESSOR_WORKSTREAM_DUPLICATE:{wid}"); continue
        by_id[wid] = row
    ...
    for wid in EXPECTED_WORKSTREAMS:
        ...
        for role, path, blob_key in (
            ...
            if _safe_path(path) != path:
                p.append(f"INTEGRATED_SUCCESSOR_PATH_INVALID:{wid}:{role}"); continue
            try:
                data = _read(root, path)
            except Exception:
                p.append(f"INTEGRATED_SUCCESSOR_BOUND_FILE_MISSING:{wid}:{role}"); continue
            actual_blob = git_blob_sha(data)
            if row.get(blob_key) != actual_blob:
                p.append(f"INTEGRATED_SUCCESSOR_BLOB_MISMATCH:{wid}:{role}")
            bound.append({"workstream_id": wid, "role": role, "path": path, "git_blob_sha": actual_blob, "raw_sha256": raw_sha256(data)})

    checks = manifest.get("mandatory_v6_adversarial_checks")
    if not isinstance(checks, list):
        checks = []; p.append("INTEGRATED_SUCCESSOR_MANDATORY_CHECKS_REQUIRED")
...
def construction_frontier() -> dict[str, Any]:
    return {"state": "V24_V6_R9_INTEGRATED_SUCCESSOR_CONSTRUCTION_READY", "qualified": False, "implementation_workstream": "R9", "scientific_execution_state": SCIENTIFIC_EXECUTION_CLOSED, "authority_effect": AUTHORITY_EFFECT}
