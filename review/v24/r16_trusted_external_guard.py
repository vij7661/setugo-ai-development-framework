...
def runtime_contract() -> dict:
    ...
    if any(v is not True for v in flags.values()):
        raise SystemExit(f"R16_GUARD_INTERPRETER_FLAGS_INVALID:{flags}")
    if sys.flags.optimize != 0:
        raise SystemExit("R16_GUARD_OPTIMIZATION_FORBIDDEN")
    exe = Path(sys.executable).resolve()
    if not exe.is_file():
        raise SystemExit("R16_GUARD_INTERPRETER_MISSING")
    r = {
        "flags": flags,
        ...
        "stdlib_names_digest": sha256(canonical(sorted(sys.stdlib_module_names))),
    }
    r["contract_digest"] = sha256(canonical(r))
    return r


def safe(rel: str) -> str:
    p = PurePosixPath(rel)
    if not rel or p.is_absolute() or ".." in p.parts or any(x in {"", "."} for x in p.parts):
        raise SystemExit(f"R16_GUARD_UNSAFE_PATH:{rel}")
    return p.as_posix()
...
def checked(root: Path, rel: str, role: str) -> dict:
    ...
    p = root / rel
    if p.is_symlink():
        raise SystemExit(f"R16_GUARD_SYMLINK:{rel}")
    if not p.is_file():
        raise SystemExit(f"R16_GUARD_FILE_MISSING:{rel}")
    if p.suffix == ".py":
        if p.name in FORBIDDEN:
            raise SystemExit(f"R16_GUARD_BOOTSTRAP_SHADOW:{rel}")
        if p.stem in sys.stdlib_module_names:
            raise SystemExit(f"R16_GUARD_STDLIB_SHADOW:{rel}")
    data = p.read_bytes()
    return {
        "path": rel,
...
def source_rows(root: Path) -> list[dict]:
    ...
    if not scope.is_dir():
        raise SystemExit("R16_GUARD_RUNTIME_SCOPE_MISSING")
    rows: list[dict] = []
    for p in sorted(scope.rglob("*")):
        if p.is_symlink():
            raise SystemExit(f"R16_GUARD_SYMLINK:{p.relative_to(root).as_posix()}")
        if p.is_file():
            rel = p.relative_to(root).as_posix()
            role = "test" if p.name.startswith("test_") and p.suffix == ".py" else "runtime"
            rows.append(checked(root, rel, role))
    for rel in SUPPORT:
        rows.append(checked(root, rel, "support"))
    rows.sort(key=lambda x: x["path"])
    if len({x["path"] for x in rows}) != len(rows):
        raise SystemExit("R16_GUARD_DUPLICATE_ADMISSION")
    return rows
...
def load(path: Path) -> dict:
    ...
    if obj.get("schema_version") != 5:
        raise SystemExit("R16_GUARD_PINSET_SCHEMA_INVALID")
    if obj.get("authority_origin") != "EXTERNAL_REVIEW_BRANCH" or obj.get("candidate_self_grant") is not False:
        raise SystemExit("R16_GUARD_AUTHORITY_INVALID")
    if obj.get("guard_runtime_contract") != runtime_contract():
        raise SystemExit("R16_GUARD_RUNTIME_CONTRACT_MISMATCH")
    return obj


def identity(obj: dict, commit: str, tree: str) -> None:
    if obj.get("candidate_commit") != commit:
        raise SystemExit("R16_GUARD_COMMIT_MISMATCH")
    if obj.get("candidate_tree") != tree:
        raise SystemExit("R16_GUARD_TREE_MISMATCH")
    if tuple(obj.get("required_support_files", ())) != SUPPORT:
        raise SystemExit("R16_GUARD_SUPPORT_SET_MISMATCH")
...
def build(args) -> None:
    ...
    print(f"R16_PINSET_FILES={len(rows)}")
    print(f"R16_PINSET_SHA256={sha256(out.read_bytes())}")
    print(f"R16_GUARD_RUNTIME_DIGEST={rt['contract_digest']}")


def verify_source(args) -> None:
    root = Path(args.root).resolve()
    obj = load(Path(args.pinset))
    identity(obj, args.commit, args.tree)
    actual = source_rows(root)
    if actual != obj.get("admitted_files"):
        raise SystemExit("R16_GUARD_SOURCE_BINDING_MISMATCH")
    print(f"R16_SOURCE_BINDING_PASS={len(actual)}")
...
def stage(args) -> None:
    ...
    identity(obj, args.commit, args.tree)
    if target.exists():
        raise SystemExit("R16_GUARD_STAGE_TARGET_EXISTS")
    target.mkdir(parents=True)
    for row in obj["admitted_files"]:
        now = checked(source, row["path"], row["role"])
        if now != row:
            raise SystemExit(f"R16_GUARD_SOURCE_CHANGED:{row['path']}")
        dst = target / row["path"]
        dst.parent.mkdir(parents=True, exist_ok=True)
        shutil.copyfile(source / row["path"], dst)
    print("R16_STAGE_PASS")
...
def sandbox_rows(root: Path) -> list[dict]:
    ...
    for p in sorted(root.rglob("*")):
        if p.is_symlink():
            raise SystemExit(f"R16_GUARD_SANDBOX_SYMLINK:{p.relative_to(root).as_posix()}")
        if p.is_file():
            rel = p.relative_to(root).as_posix()
            data = p.read_bytes()
            rows.append({
...
def verify_sandbox(args) -> None:
    root = Path(args.root).resolve()
    obj = load(Path(args.pinset))
    identity(obj, args.commit, args.tree)
    exp = {
        r["path"]: {k: r[k] for k in ("path", "git_blob_sha1", "raw_sha256", "bytes")}
        for r in obj["admitted_files"]
    }
    act = {r["path"]: r for r in sandbox_rows(root)}
    if set(exp) != set(act):
        raise SystemExit(
            f"R16_GUARD_SANDBOX_FILESET_MISMATCH:missing={sorted(set(exp)-set(act))}:extra={sorted(set(act)-set(exp))}"
        )
    for path in sorted(exp):
        if exp[path] != act[path]:
            raise SystemExit(f"R16_GUARD_SANDBOX_BINDING_MISMATCH:{path}")
    print(f"R16_SANDBOX_BINDING_PASS={len(act)}")
