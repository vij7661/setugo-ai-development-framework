...
def runtime_contract()->dict:
    ...
    if any(v is not True for v in flags.values()):raise SystemExit(f"R14_GUARD_INTERPRETER_FLAGS_INVALID:{flags}")
    exe=Path(sys.executable).resolve()
    if not exe.is_file():raise SystemExit("R14_GUARD_INTERPRETER_MISSING")
...
def safe(rel:str)->str:
    p=PurePosixPath(rel)
    if not rel or p.is_absolute() or ".." in p.parts or any(x in {"","."} for x in p.parts):raise SystemExit(f"R14_GUARD_UNSAFE_PATH:{rel}")
    return p.as_posix()

def checked(root:Path,rel:str,role:str)->dict:
    rel=safe(rel);p=root/rel
    if p.is_symlink():raise SystemExit(f"R14_GUARD_SYMLINK:{rel}")
    if not p.is_file():raise SystemExit(f"R14_GUARD_FILE_MISSING:{rel}")
    if p.suffix==".py":
        if p.name in FORBIDDEN:raise SystemExit(f"R14_GUARD_BOOTSTRAP_SHADOW:{rel}")
        if p.stem in sys.stdlib_module_names:raise SystemExit(f"R14_GUARD_STDLIB_SHADOW:{rel}")
...
def source_rows(root:Path)->list[dict]:
    ...
    if not scope.is_dir():raise SystemExit("R14_GUARD_RUNTIME_SCOPE_MISSING")
    rows=[]
    for p in sorted(scope.rglob("*")):
        if p.is_symlink():raise SystemExit(f"R14_GUARD_SYMLINK:{p.relative_to(root).as_posix()}")
        if p.is_file():
    ...
    if len({x["path"] for x in rows})!=len(rows):raise SystemExit("R14_GUARD_DUPLICATE_ADMISSION")
    return rows

def load(path:Path)->dict:
    obj=json.loads(path.read_text())
    if obj.get("schema_version")!=3:raise SystemExit("R14_GUARD_PINSET_SCHEMA_INVALID")
    if obj.get("authority_origin")!="EXTERNAL_REVIEW_BRANCH" or obj.get("candidate_self_grant") is not False:raise SystemExit("R14_GUARD_AUTHORITY_INVALID")
    if obj.get("guard_runtime_contract")!=runtime_contract():raise SystemExit("R14_GUARD_RUNTIME_CONTRACT_MISMATCH")
    return obj

def identity(obj:dict,commit:str,tree:str)->None:
    if obj.get("candidate_commit")!=commit:raise SystemExit("R14_GUARD_COMMIT_MISMATCH")
    if obj.get("candidate_tree")!=tree:raise SystemExit("R14_GUARD_TREE_MISMATCH")
    if tuple(obj.get("required_support_files",()))!=SUPPORT:raise SystemExit("R14_GUARD_SUPPORT_SET_MISMATCH")

def build(args):
    root=Path(args.root).resolve();rows=source_rows(root);rt=runtime_contract()
    obj={"schema_version":3,"authority_origin":"EXTERNAL_REVIEW_BRANCH","candidate_self_grant":False,"candidate_commit":args.commit,"candidate_tree":args.tree,"review_commit":args.review_commit,"guard_runtime_contract":rt,"admitted_files":rows,"required_support_files":list(SUPPORT),"scientific_execution_state":"CLOSED_PENDING_SUCCESSOR_REVIEW","authority_effect":AUTHORITY_EFFECT}
    out=Path(args.output);out.write_text(json.dumps(obj,indent=2,sort_keys=True)+"\n")
    print(f"R14_PINSET_FILES={len(rows)}");print(f"R14_PINSET_SHA256={sha256(out.read_bytes())}");print(f"R14_GUARD_RUNTIME_DIGEST={rt['contract_digest']}")
def verify_source(args):
    ...
    if actual!=obj.get("admitted_files"):raise SystemExit("R14_GUARD_SOURCE_BINDING_MISMATCH")
    print(f"R14_SOURCE_BINDING_PASS={len(actual)}")
def stage(args):
    ...
    if target.exists():raise SystemExit("R14_GUARD_STAGE_TARGET_EXISTS")
    target.mkdir(parents=True)
    for row in obj["admitted_files"]:
        now=checked(source,row["path"],row["role"])
        if now!=row:raise SystemExit(f"R14_GUARD_SOURCE_CHANGED:{row['path']}")
        dst=target/row["path"];dst.parent.mkdir(parents=True,exist_ok=True);shutil.copyfile(source/row["path"],dst)
    print("R14_STAGE_PASS")
def sandbox_rows(root:Path)->list[dict]:
    rows=[]
    for p in sorted(root.rglob("*")):
        if p.is_symlink():raise SystemExit(f"R14_GUARD_SANDBOX_SYMLINK:{p.relative_to(root).as_posix()}")
        if p.is_file():
...
def verify_sandbox(args):
    ...
    if set(exp)!=set(act):raise SystemExit(f"R14_GUARD_SANDBOX_FILESET_MISMATCH:missing={sorted(set(exp)-set(act))}:extra={sorted(set(act)-set(exp))}")
    for path in sorted(exp):
        if exp[path]!=act[path]:raise SystemExit(f"R14_GUARD_SANDBOX_BINDING_MISMATCH:{path}")
    print(f"R14_SANDBOX_BINDING_PASS={len(act)}")
