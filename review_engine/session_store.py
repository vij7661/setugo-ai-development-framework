...
class SQLiteSessionStore:
    ...
    def __init__(self, path: str | Path) -> None:
        self.path = str(path); self._init_schema()
    ...
    def append(self, session_id: str, event_type: str, payload: dict) -> SessionEvent:
        if not session_id or not event_type: raise ValueError("session_id and event_type required")
        if not isinstance(payload, dict): raise ValueError("payload must be an object")
        conn = self._connect()
        try:
            conn.execute("BEGIN IMMEDIATE")
            latest = conn.execute("SELECT seq, event_hash FROM review_events WHERE session_id=? ORDER BY seq DESC LIMIT 1", (session_id,)).fetchone()
            ...
            row = conn.execute("SELECT * FROM review_events WHERE session_id=? AND seq=?", (session_id, seq)).fetchone()
            conn.commit()
        except Exception:
            conn.rollback(); raise
        finally:
    ...
    def _from_row(row: sqlite3.Row) -> SessionEvent:
        return SessionEvent(
            session_id=row["session_id"], seq=int(row["seq"]), event_type=row["event_type"],
            payload=json.loads(row["payload_json"]), previous_hash=row["previous_hash"],
            event_hash=row["event_hash"], created_at=row["created_at"],
        )
    ...
    def validate_chain(self, session_id: str) -> bool:
        previous = GENESIS; expected_seq = 1
        for event in self.events(session_id):
            if event.seq != expected_seq or event.previous_hash != previous: return False
            expected_hash = _event_hash(event.session_id, event.seq, event.event_type, event.payload, event.previous_hash)
            if event.event_hash != expected_hash: return False
            previous = event.event_hash; expected_seq += 1
        return True

    def latest_decision(self, session_id: str) -> SessionEvent | None:
        with self._connect() as conn:
            row = conn.execute("SELECT * FROM review_events WHERE session_id=? AND event_type='FINAL_DECISION' ORDER BY seq DESC LIMIT 1", (session_id,)).fetchone()
    ...
    def list_sessions(self, *, limit: int = 100) -> tuple[SessionSummary, ...]:
        ...
        for row in rows:
            ...
            summaries.append(SessionSummary(
                session_id=session_id, started_at_utc=row["started_at"], updated_at_utc=row["updated_at"],
                event_count=int(row["event_count"]), final_state=payload.get("state"),
                final_reasons=tuple(str(v) for v in payload.get("reasons", [])),
                artifact_hash=payload.get("artifact_hash"), chain_valid=self.validate_chain(session_id),
            ))
