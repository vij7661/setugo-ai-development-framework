...
def test_low_risk_request_finishes_with_r1_only():
    calls = []
    ...
    assert result.state == "CONVERGED_PASS"
    assert result.final_output == "simple answer"
    assert [c[0] for c in calls] == ["R1"]


def test_platform_high_risk_floor_cannot_be_downgraded_by_r1():
    calls = []
...
def test_r1_can_escalate_review_but_not_self_authorize_direct_finalization():
    calls = []
...
def test_material_r2_finding_causes_scoped_revision_and_blinded_r3_verification():
    calls = []
    ...
    def invoke(config, context):
        ...
        if config.role == "R2":
            finding = ReviewFinding(
                ...
                severity="HIGH",
                material=True,
                summary="claim 2 violates invariant AUTH-1",
                violated_invariant="AUTH-1",
                affected_scope=("claim:2",),
                first_invalid_claim="claim:2",
            )
            return ReviewerResponse(
                role="R2",
                artifact_hash=context["artifact"]["artifact_hash"],
                output="localized finding",
                findings=(finding,),
            )
        if config.role == "R1":
            assert context["mode"] == "SCOPED_CORRECTION"
            assert context["verified_review_targets"][0]["affected_scope"] == ["claim:2"]
            return ReviewerResponse(role="R1", artifact_hash=None, output="v2 corrected")
        assert config.role == "R3"
        assert context["phase"] == "INDEPENDENT"
        assert "prior_reviews" not in context
        return ReviewerResponse(
            role="R3",
            artifact_hash=context["artifact"]["artifact_hash"],
    ...
    assert result.state == "CONVERGED_PASS"
    assert result.final_output == "v2 corrected"
    assert calls == [
        ("R1", None, None),
        ("R2", None, None),
        ("R1", "SCOPED_CORRECTION", None),
        ("R3", None, "INDEPENDENT"),
    ]


def test_high_risk_r3_must_be_lineage_independent_from_r2():
    def invoke(config, context):
        if config.role == "R1" and context.get("mode") != "SCOPED_CORRECTION":
        ...
        raise AssertionError("R3 must not be called")

    result = ReviewEngine(invoke).run(
        ReviewRequest(request_id="q5", user_input="critical change", risk="HIGH", materiality="MATERIAL"),
        r1=cfg("R1"),
        r2=cfg("R2", lineage="same"),
        r3=cfg("R3", lineage="same"),
    )
    assert result.state == "HUMAN_REQUIRED"
    assert "foundation-lineage" in result.reasons[0]


def test_r3_prior_reviews_are_disclosed_only_after_independent_view_is_frozen():
    phases = []
    ...
    def invoke(config, context):
        ...
        assert context["phase"] == "ADJUDICATION"
        assert context["frozen_independent_view"] == "independent concern"
        assert context["prior_reviews"]["R2"] == "R2 says issue"
    ...
    assert result.state == "CONVERGED_PASS"
    assert phases == ["INDEPENDENT", "ADJUDICATION"]


def test_authoritative_memory_is_versioned_and_external_authority_only():
...
def test_incomplete_consequential_evidence_fails_to_human_before_r2():
    calls = []
