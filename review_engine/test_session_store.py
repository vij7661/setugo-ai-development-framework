...
class SessionStoreTests(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(); self.path = Path(self.tmp.name) / "sessions.db"; self.store = SQLiteSessionStore(self.path)
    ...
    def test_session_events_are_hash_linked_and_persistent(self):
        ...
        self.assertEqual(e1.seq, 1); self.assertEqual(e2.seq, 2); self.assertEqual(e2.previous_hash, e1.event_hash)
        reopened = SQLiteSessionStore(self.path)
        self.assertTrue(reopened.validate_chain("s1")); self.assertEqual(len(reopened.events("s1")), 2)
