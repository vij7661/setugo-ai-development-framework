...
from .configuration import build_provider_registry, load_configuration
...
def build_request(payload: dict) -> ReviewRequest:
    ...
    if materiality is not None:
        ...
        if materiality_order[materiality] > materiality_order[facts.materiality_floor]:
            effective_materiality = materiality
        else:
    ...
    return ReviewRequest(
        ...
        risk=facts.risk_floor,
        materiality=effective_materiality,
        external_action=facts.external_action,
...
def run_from_files(*, config_path: str, request_path: str, memory_db: str, sessions_db: str) -> dict:
    ...
    registry = build_provider_registry(configuration)
    ...
    engine = ReviewEngine(registry.invoke, session_store=sessions)
    decision = engine.run(
        request,
        r1=configuration.reviewer("R1"),
        r2=configuration.reviewer("R2"),
        r3=configuration.reviewer("R3"),
        memory=memory,
    )
    result = asdict(decision)
    result["request_id"] = request.request_id
    result["platform_facts"] = request.platform_facts
    result["action_authorized"] = False
    result["session_chain_valid"] = sessions.validate_chain(request.request_id)
    return result
